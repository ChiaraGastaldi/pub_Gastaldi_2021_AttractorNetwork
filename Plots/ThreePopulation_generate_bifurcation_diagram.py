...
resolution_factor = 1.1  #playes with the precision of the nullclines
...
def extract_critical_C_hat(data_cells):
    ...
    for i in range(len(x_p)):
        if x_p[i] > 0.9    and stab[i] == 1.0:
            ...
            C_hat_crit = C_hat[i]
    return C_hat_crit
data = np.loadtxt("../Output_3pop/Bifurcation_plot_vs_C_gamma%s_size%s_resolution%s_factor%s_rm%s_A%s_h0%s_b%s_load%s.dat"%(gamma, size, resolution, resolution_factor, rm, A, h0, b,  load), unpack=True, skiprows=1, delimiter = ",")
...
plt.savefig( "../Figures_3pop/Bifurcation_vs_c_size%s_resolution_pp_%s_resolution_factor_%s_rm%s_A%s_h0%s_b%s_gamma%s_load%s_model%s.pdf" %(size, resolution, resolution_factor, rm, A, h0, b, gamma, load, model),   transparent=True)
plt.show()
