...
data = np.loadtxt("../Output_full_sim_adapt_distr_gains/adapt_overlap_time_neus%s_gamma%s_P%s_tau%s_dt%s_chat%s_A%s_b%s.dat"%(N, gamma, P, tau, h, C_hat, A,  b), unpack=True)
...
C_hat=0.15
data = np.loadtxt("../Output_full_sim_adapt_distr_gains/adapt_overlap_time_neus%s_gamma%s_P%s_tau%s_dt%s_chat%s_A%s_b%s.dat"%(N, gamma, P, tau, h, C_hat, A,  b), unpack=True)
#time, m1, m2, stab = np.loadtxt("../../AttractorNetwork/output_2pop_adapt/2pop_with_noise_vs_time_gamma%s_time_step%s_resolution%s_factor%s_rm%s_A%s_h0%s_b%s_load%s_Chat%s.dat"%(gamma, h, resolution, resolution_factor, rm, A, h0, b, load, C_hat), unpack = True, skiprows=1, delimiter = ",")
for i in range(1,P+1):
...
"""
C_hat=0.2
data = np.loadtxt("../Output_full_sim_adapt_distr_gains/adapt_overlap_time_neus%s_gamma%s_P%s_tau%s_dt%s_chat%s_A%s_rm%s_b%s.dat"%(N, gamma, P, tau, h, C_hat, A, rm, b), unpack=True)
time, m1, m2, stab = np.loadtxt("../../AttractorNetwork/output_2pop/2pop_with_noise_vs_time_gamma%s_time_step%s_resolution%s_factor%s_rm%s_A%s_h0%s_b%s_load%s_Chat%s.dat"%(gamma, h, resolution, resolution_factor, rm, A, h0, b, load, C_hat), unpack = True, skiprows=1, delimiter = ",")
for i in range(1,P+1):
...
sigma = 0.0141230
noise_distr = np.loadtxt("../Output_full_sim_adapt_distr_gains/final_noise_ditr_N%s_gamma%s_P%s_tau%s_dt%s_chat%s_A%s_b%s.dat"%(N, gamma, P, tau, h, C_hat, A, b), unpack=True)
